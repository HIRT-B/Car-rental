...
export default function StatusCard({ title, count, description, status }) {
  ...
  // Format count if it's a number (like revenue)
  const formattedCount =
    typeof count === 'number' ? `${count.toFixed(2)} Dh` : count;
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: '#111',
        ...style,
        boxShadow: '0 4px 10px rgba(244, 192, 76, 0.15)',
        transition: 'all 0.3s ease',
        borderRadius: '12px',
      }}
      onMouseEnter={(e) => {
        e.currentTarget.style.boxShadow =
          '0 0 20px rgba(244, 192, 76, 0.35)';
        e.currentTarget.style.transform = 'translateY(-5px)';
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.boxShadow =
          '0 4px 10px rgba(244, 192, 76, 0.15)';
        e.currentTarget.style.transform = 'translateY(0)';
      }}
    >
      <div className="card-body text-center">
        <h5 className="card-title fw-bold">{title}</h5>
        <h2 className="text-light fw-bolder">{formattedCount}</h2>
