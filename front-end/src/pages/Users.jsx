  ...
  const handleBan = (userId) => {
    if (window.confirm("هل أنت متأكد من حظر هذا المستخدم؟")) {
      axios
        ...
        .then(() => {
          setUsers((prev) =>
            prev.map((u) => (u.user_id === userId ? { ...u, status: "banned" } : u))
          );
        })
      ...
      <div className="container">
        <div className="d-flex justify-content-between">
          <div>
            <h1
              className="fw-bold"
              style={{ fontWeight: "700", color: "#f4c04c", marginTop: "2rem" }}
            >
              Customer Management
            </h1>
        ...
        <div className="row mt-5">
          <div className="col-md-3 mb-4">
            <StatusCard title="Total Customers" count={stats.totalUsers} status="info" />
          </div>
          <div className="col-md-3 mb-4">
            <StatusCard title="Active Users" count={stats.activeUsers} status="success" />
          </div>
          <div className="col-md-3 mb-4">
            <StatusCard title="Inactive Users" count={stats.inactiveUsers} status="warning" />
          </div>
          <div className="col-md-3 mb-4">
            <StatusCard title="Banned Users" count={stats.bannedUsers} status="danger" />
          </div>
        </div>
...
>
  <thead style={{ backgroundColor: "#1a1a1a", color: "#f4c04c" }}>
    <tr>
      <th style={{ padding: "16px", fontSize: "0.9rem", textTransform: "uppercase" }}>Customer</th>
      <th style={{ padding: "16px", fontSize: "0.9rem", textTransform: "uppercase" }}>Contact</th>
      <th style={{ padding: "16px", fontSize: "0.9rem", textTransform: "uppercase" }}>Rental History</th>
      <th style={{ padding: "16px", fontSize: "0.9rem", textTransform: "uppercase" }}>Total Spent</th>
      <th style={{ padding: "16px", fontSize: "0.9rem", textTransform: "uppercase" }}>Status</th>
      <th style={{ padding: "16px", fontSize: "0.9rem", textTransform: "uppercase" }}>Actions</th>
    </tr>
  </thead>
  <tbody>
    {rentals
      ...
      rentals
        ...
        .map((user) => (
          <tr key={user.user_id} style={{ borderBottom: "1px solid #222" }}>
            <td style={{ padding: "16px", color: "#ddd", verticalAlign: "middle" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: "0.8em", color: "#aaa" }}>
                    ID: {user.customerID || "KM00" + user.user_id}
                  </div>
                </div>
              </div>
            </td>
            ...
            <td style={{ padding: "16px", color: "#ddd", verticalAlign: "middle" }}>
              <Status bgstatus={user.status} typestatus={user.status} mode="table" />
            </td>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p>
                  Voulez-vous vraiment supprimer <strong>{deleteUserName}</strong> ?
                </p>
              </div>
